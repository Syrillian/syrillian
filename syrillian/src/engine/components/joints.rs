...
use rapier3d::{math::{Isometry, Vector}, prelude::{FixedJointBuilder, GenericJoint, ImpulseJointHandle, JointAxis, PrismaticJointBuilder, RevoluteJointBuilder, RigidBody, RopeJointBuilder, SphericalJointBuilder, SpringJointBuilder}};
...
impl Default for RevoluteConfig {
    fn default() -> Self {
        Self { axis: Vector3::y_axis(), limits: None }
    }
}
...
impl Default for SpringConfig {
    fn default() -> Self {
        Self { rest_length: 1.0, stiffness: 100.0, damping: 10.0 }
    }
}
...
impl <T: JointTypeTrait> JointComponent<T> {
    ...
    pub fn try_connect_to(&mut self, body: GameObjectId) -> Result<(), JointError> {
        ...
        self.handle = Some(
            self.parent.world().physics.impulse_joint_set.insert(self_rb, other_rb, joint, true)
        );
    ...
    pub fn joint_data(&self) -> Option<&GenericJoint> {
        Some(&self.parent.world().physics.impulse_joint_set.get(self.handle?)?.data)
    }
    
    pub fn joint_data_mut(&self) -> Option<&mut GenericJoint> {
        Some(&mut World::instance().physics.impulse_joint_set.get_mut(self.handle?, false)?.data)
    }
    ...
    pub fn check_break(&mut self, world: &mut World) {
        let dominated = self.break_force.is_some_and(|t| {
            self.force_magnitude().is_some_and(|f| f > t)
        }) || self.break_torque.is_some_and(|t| {
            self.torque_magnitude().is_some_and(|f| f > t)
        });
...
impl JointComponent<Revolute> {
    ...
    pub fn set_limits(&mut self, min: f32, max: f32) {
        ...
        if let Some(j) = self.joint_data_mut() {
            if let Some(r) = j.as_revolute_mut() {
    ...
    pub fn angle(&self) -> Option<f32> {
        let (rb1, rb2) = self.bodies()?;
        self.joint_data()?.as_revolute()?.angle(rb1.rotation(), rb2.rotation()).into()
    }
...
impl JointComponent<Rope> {
    ...
    pub fn is_taut(&self) -> bool {
        self.current_distance().is_some_and(|d| d >= self.config.max_distance - 0.01)
    }
    ...
    pub fn slack(&self) -> Option<f32> {
        self.current_distance().map(|d| (self.config.max_distance - d).max(0.0))
    }
    
    pub fn tension_ratio(&self) -> Option<f32> {
        self.current_distance().map(|d| (d / self.config.max_distance).min(1.0))
    }
...
impl JointComponent<Spring> {
    ...
    pub fn potential_energy(&self) -> Option<f32> {
        self.extension().map(|x| 0.5 * self.config.stiffness * x * x)
    }
